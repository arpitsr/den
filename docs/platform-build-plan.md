...
17. [ ] Multi-tenant: API-key table + `owner` column use, per-key quotas
   ...
   Auth decision: keys/quotas are den-native (SQLite; CLI and CI clients
   need long-lived keys no auth SaaS mints for you). Human identity for
   the web UI goes through a provider in FRONT of the platform (WorkOS
   AuthKit / Clerk) — den never speaks OIDC; the provider session mints a
   den API key via the platform backend. Session-scoped attach tokens
   stay den-minted and independent of the identity provider.
